-- CETADMI - CERTIFICATE CODE SHORTENING
-- Purpose: use intuitive short certificate codes derived from registration ids.

CREATE OR REPLACE FUNCTION public.generate_certificate_code_from_registration(p_registration_id UUID)
RETURNS TEXT AS $$
BEGIN
    RETURN upper(substr(replace(p_registration_id::text, '-', ''), 1, 8));
END;
...
BEGIN
    ...
    INSERT INTO public.certificate_issues (
        ...
        public.generate_certificate_code_from_registration(target_registration.id),
